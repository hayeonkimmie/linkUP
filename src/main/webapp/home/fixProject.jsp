...
<head>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const statusButtons = document.querySelectorAll('.recruit-status');
      let skipValidationOnce = false;

      statusButtons.forEach(btn => {
        btn.addEventListener('click', () => {
          statusButtons.forEach(b => b.classList.remove('btn-selected'));
          btn.classList.add('btn-selected');
          hiddenStatusInput.value = btn.dataset.status;
        });
      });
      // 산업 데이터 (공통)
      const industryData = {
...
<div class="bg-gray-50 p-6">
  ...
  <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow">
    <h1 class="text-2xl font-bold mb-4">구인 수정</h1>
    <form id="job-form" class="space-y-6">
      ...
      <section>
        ...
        <div class="mt-4">
          <span class="block font-medium mb-2">구인 상태 <span class="text-red-500">*</span></span>
          <div class="flex space-x-2">
            <button type="button" data-status="open" class="recruit-status px-4 py-2 border rounded">구인중</button>
            <button type="button" data-status="closed" class="recruit-status px-4 py-2 border rounded">구인 완료</button>
          </div>

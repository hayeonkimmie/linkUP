...
// 폼 제출 전 최종 데이터 준비 함수 개선
    ...
    /*    // 카테고리 정보 확인
        ...
        // 화면에 표시된 카테고리가 있지만 hidden 값이 없는 경우 처리
        if (!categoryHidden.value && specializationSpan && specializationSpan.textContent) {
            // 서브카테고리 선택 이벤트를 강제로 트리거하여 hidden 값 설정
            const subCategorySelect = document.getElementById("subCategory");
            if (subCategorySelect && subCategorySelect.value) {
                categoryHidden.value = subCategorySelect.value;
            }
        }
        ...
        // 동적 섹션 인덱스 재정렬 - 서블릿에서 올바른 인덱스 순서로 파라미터를 읽을 수 있도록
        renumberAll();

        // 파일과 URL 인덱스 정리 - 리스트 형태로 서블릿에서 처리하기 쉽게 변경
...
// 폼 제출 이벤트에 연결
    ...
    /*function prepareFormDataForSubmission() {
        /!*!// 카테고리 정보 확인
        const categoryHidden = document.getElementById("categoryHidden");
        if (!categoryHidden.value && document.querySelector(".specialization span")) {
            // 서버에서 가져온 기존 카테고리가 있고 사용자가 새로 선택하지 않은 경우
            console.log("카테고리 정보 유지: 사용자가 변경하지 않음");
        }
        ...
        // 첨부파일 처리
        updateFileIndexes();*!/
        // 각 섹션의 데이터를 JSON으로 변환하여 히든 필드에 저장

        // 스킬 데이터
        const skillTags = document.querySelectorAll('.skill-tag');
        const skillData = Array.from(skillTags).map(tag => tag.getAttribute('data-skill'));
        document.getElementById('skillDescriptionHidden').value = skillData.join(',');

        // 카테고리 정보
        const categoryHidden = document.getElementById("categoryHidden");
        if (!categoryHidden.value) {
            const mainCategoryId = document.getElementById('category_name').value;
